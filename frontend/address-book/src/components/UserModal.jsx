...
const UserModal = ({ showModal, setShowModal }) => {
	...
	const [modalEditableState, setModalEditableState] = useRecoilState(
		currentModalEditState
	);
	...
	const deleteUser = () => {
		axios
			.delete(
				`http://localhost:5172/api/customers/deleteCustomer/${userInfo._id}`
			)
			.then(() => {
			...
			.catch(err => {
				console.log({
					msg: `Error deleting User ${userInfo.firstName} ${userInfo.lastName}`,
					err: err,
				});
			});
	};
	...
	const onSubmitHandler = e => {
		...
		if (!userInfo._id) {
			axios
				.post(`http://localhost:5172/api/customers/createCustomer`, {
					userInfo,
				})
				.then(() => {
					setUserInfo({
			...
			axios
				.get(`http://localhost:5172/api/customers/findCustomer/${userInfo._id}`)
				.then(() => {
					axios
						.put(
							`http://localhost:5172/api/customers/editCustomer/${userInfo._id}`,
							{ userInfo }
						)
						.then(res => {
			...
			<div className="modalContent" onClick={e => e.stopPropagation()}>
				...
				<form onSubmit={onSubmitHandler}>
					...
					{userInfo._id ? (
						<button
							className={`${
								modalEditableState ? 'modalButtonDisabled' : 'modalButton'
							} `}
							onClick={deleteUser}
							disabled={modalEditableState}
						>
